...
Izkaže se, da lahko z elementarnim dokazom dokažemo nekoliko močnejšo obliko komutatorske leme. Najprej moramo vpeljati naslednjo definicijo.
...
\begin{opomba}
Beseda $w \in F_k$ je zakon v podmnožici $H \subseteq G$ natanko tedaj, ko je $Z(G, w) \supseteq H^{k}$ ter zakon v vsaki podmnožici $(H_i)_{i = 1}^{n}$ natanko tedaj, ko je $Z(G, w) \supseteq \bigcup_{i = 1}^{n} H_i^{k}$.  
\end{opomba}
% TODO malo popravi tole opombo

\begin{lema}
\label{lem_komutaroska_lema_nova}
Naj bo $G$ grupa, $H_i \subseteq  G$ njene simetrične podmnožice ($H_i = H_i ^{-1}$), in naj bodo $w_{i} \in F_k$ zakon v podmnožici $H_i$ za $i = 1, \ldots, 2^{e}$. Potem obstaja beseda $w \in F_k$ dolžine \begin{equation*}
l(w) \le  m \sum_{i = 1}^{n} l(w_i),
\end{equation*}  
za katero velja $H_1 \cup  \ldots \cup H_n \subseteq  Z(G, w)$, torej je zakon v vseh podgrupah $H_i$. 
\end{lema}
\begin{dokaz}
Ideja dokaza je pokazati, da lahko v dokazu leme \ref{lem_komutatorska_lema} lahko bolj elementarno -- brez uporabe Nielsenovega izreka -- pokažemo netrivialnost komutatorjev.
...
\begin{posledica}
\label{psl_komutatorska_lema_splosna}
...
Čeprav je ta primer enostaven, je ključ do praktično vseh konstrukcij zakonov za družine, kar bomo videli recimo na koncu razdelka \ref{sec_grupe_psl2q} o grupah $\operatorname{PSL}_2(q)$. 
...
\begin{definicija}
\label{def_zakon_v_podmnožici}
Naj bo $G$ grupa in $H$ njena podmnožica. Beseda $w$ je zakon v podmnožici $H$, če za vsak par $g, h \in H$ velja $w(g,h) = 1_G$.
\end{definicija}

\begin{lema}
\label{lem_komutaroska_lema_nova}
Naj bo $G$ grupa, $H_i \subseteq  G$ njene podmnožice in naj bodo $w_{i} \in F_k$ netrivialni zakoni v podmnožici $H_i$ za $i = 1, \ldots, n$. Potem obstaja beseda $w \in F_k$ dolžine \begin{equation*}
l(w) \le 4 \sum_{i = 1}^{n} l(w_i),
\end{equation*}  
ki je zakon v podmnožici $H_1 \cup  \ldots \cup H_n$. 
\end{lema}
\begin{dokaz}
Ideja dokaza je pokazati, da se lahko izognemo tehnični zahtevnosti v dokazu leme \ref{lem_komutatorska_lema}, saj lahko zakone za posamezne podmnožice med seboj združujemo na pameten način.

\end{dokaz}


% TODO dodaj nekaj primerov uporabe komutatorske in razširitvene leme (na produktu in na diedrskih grupah)
% oba ta primera vsebinsko poveži - komutatorska - s problemom, razširitvena - z enostavnimi, nilpotentnimi in rešljivimi grupami + virtualno nilpotentnimi grupami
...
% TODO v resnici je bolj smiselno, če ta sklep spada na konec poglavja, saj skupaj poveže vse posamezne ugotovitve
...
Avtorja članka \cite{Kozma_Thom_2016} sta rezultat močno izboljšala in sicer na obliko \begin{equation*}
    \alpha(n)  \le \exp((1 + o(1)) \log(n)^4 \log (\log n))
\end{equation*}
z uporabo Liebeckovega izreka, ki opredeli strukturo po
...
\begin{definicija}\label{def_pslnq_in_psl2q}
    ...
    Druga taka grupa je recimo $\operatorname{SL}_2(\mathbb{Z})$, saj vsebuje vse grupe $\operatorname{PSL}_2(q)$ kot kvociente (TODO najdi vir in razloži). 
...
\end{dokaz}
\noindent
    Za konkretno grupo $G = \operatorname{PSL}_2(q)$ definirajmo podmnožice $H_m := \left\{ A \in \operatorname{PSL}_2(q)  \middle|\,  A^{m} = I \right\}$ za števila $m \in \left\{ p, q-1 , q+1\right\}$.
    Po razmisleku iz prejšnje leme te podmnožice tvorijo pokritje $G$. Z uporabo komutatorske leme (TODO tu se moraš sklicati na svojo obliko !!), je netrivialni zakon v grupi $G$ oblike \begin{equation*}
    w = [[x^{p}, y^{q-1}], y^{q + 1}] = x^{p} y^{q-1} x^{-p} \cancel{ y^{1 - q}} y^{q + 1} \cancel{y^{q -1}} x^{p} y^{1 - q} x^{-p} y^{- q - 1}= x^{p} y^{q-1} x^{-p}  y^{q + 1} x^{p} y^{1 - q} x^{-p} y^{- q - 1} 
    \end{equation*}  
    dolžine \begin{equation*}
    l(w) = 4(p + q) \le 8q. 
    \end{equation*}
    ...
    \end{lema}
    Ta lema je ena izmed oblik osnovnega izreka o praštevilih, ki ga je prvi dokazal (TODO kdo, katerega leta). Ker je dokaz netrivialen, ga bomo opustili, bralec ga lahko najde v (TODO). 
