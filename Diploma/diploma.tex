...
\begin{lema}
    ...
    \begin{dokaz}
        ...
        Zdaj moramo le še utemeljiti, da lahko besedi $v_1$ ter $v_2$ ustrezno združimo. Vemo, da je komutator $[a,b]$ za $a,b \in F_k \setminus \left\{ 1\right\}$ trivialen natanko tedaj, ko $a$ in $b$ generirata prosto grupo ranga $1$. Po lemi \href{lem_uporaba_nielsen} namreč vemo, da je podgrupa $\langle a, b \rangle \le F_k$ prosta,
...
\begin{dokaz}
    ...
    \begin{enumerate}
        \item Če je $w_{ G / N } = s^{n}$ za neki $s \in S$, $n \in  \mathbb{Z} \setminus \left\{ 0\right\} $, so vse besede oblike $t^{n}$, kjer je $t \in S$, zakoni za $G / N$. Zato lahko vzamemo besedo
        \item \begin{equation*}
...
\begin{primer}
Trditev \href{trd_p_grupe_so_nilpotentne} nam sporoča, da so vse diedrske grupe oblike $D_{2 \cdot 2^{k}}$ nilpotentne. Izkaže se, da so to tudi vse, saj (TODO tukaj moraš končati razmislek z dokazom https://math.stackexchange.com/questions/834966/is-the-dihedral-group-d-n-nilpotent-solvable). 
\end{primer}

% TODO zdaj si moraš natančno prebrati prispevek o nilpotentnih grupah in razbrati bistvo. 
...
\begin{primer}
    ...
    \end{equation*}
    Niso pa vse nilpotentne grupe rešljive, primer so recimo diedrske grupe $D_2n$, kjer $2n$ ni dvojiška potenca. 
\end{primer}
...
\end{trditev}
\begin{dokaz}
    TODO večina jih je dokaj očitnih, malo težja je le četrta, vendar jo imaš v Schneider konec strani 27
    \begin{enumerate}
        \item 
    \end{enumerate}
\end{dokaz}
...
\end{trditev}
\begin{dokaz}
    Tritev je posledica razširitvene leme \href{lem_razsiritvena_lema}, dokaz poteka z indukcijo po $d$. Za $d = 1$ je grupa $G$ Abelova, zato je ustrezni zakon beseda $w = [x,y]$, ki je dolžine 4.
...
\begin{opomba}
V članku (TODO najdi ta članek !!) je podana nekoliko šibkejša meja $l(w) \le  4 \cdot 6^{d-1}$, ker je avtor uporabil šibkejšo obliko razširitvene leme.
\end{opomba}

\subsubsection{Konstrukcija zakonov za rešljive grupe}

Konstrukcija kratikih zakonov za rešljive grupe je veliko težja od konstrukcije za nilpotentne grupe, zato bom predstavil predvsem glavne ideje. 

% TODO tukaj moraš razmisliti, kaj točno sploh hočeš predstaviti in na kakšen način. Vsekakor se ne da vsega ... 
