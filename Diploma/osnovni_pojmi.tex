...
Naj bo beseda $w \in  F(S)$ oblike $w = s_1 \cdots s_n$. Potem definiramo \emph{dolžino besede} $l(w) = n$.
\end{definicija}
...
\end{dokaz}
Z uporabo te leme direktno sledi, da je grupa $K(k)$ podgrupa končnega indeksa največ $\lvert G \rvert^{ \lvert G \rvert^k}$ v $F_k$. 
...
\begin{definicija}\label{def_ozina}
    ...
    \end{equation*}  
    rečemo \emph{$k$-črkovna dolžina grupe $G$}.  
    \end{definicija}
    

\begin{opomba} TODO tole malo popravi
V nekaterih drugih virih, denimo \cite{Schneider_2016} in \cite{Kozma_Thom_2016}, temu številu rečejo $k$-črkovna ožina grupe $G$. To ime izhaja iz Cayleyjevega grafa grupe. Vsak cikel $g_1 , g_2, \ldots, g_n, g_1$ tega grafa namreč
podaja zvezo $s_1 s_2 \cdots s_n = 1_G$, kjer za $i = 2, \ldots, n$ velja $s_i = g_{i - 1}^{-1} g_i$ in dodatno še $s_1 = g_n^{-1} g_1$.
V kontekstu zakonov je to ime nekoliko zavajajoče, saj beseda $s_1 s_2 \cdots s_n$ ni nujno zakon v grupi. Če si pogledamo na primer grupo $C_3 \times C_5 = \langle (\xi, 1), (1, \eta) \rangle$,
